...
export async function updateUserAccess(userId, allowedNavKeys, screenPrivileges) {
  return request(`/users/${userId}/access`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ allowedNavKeys, screenPrivileges, isCustomized: true }),
  });
}

export async function resetUserAccess(userId) {
  return request(`/users/${userId}/access`, { method: 'DELETE' });
}
